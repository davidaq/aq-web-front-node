...
function init(callback) {
    ...
    warnExists(path.join(targetDir, targetName + '.js'), function() {
        warnExists(path.join(targetDir, targetName + '.css'), choosePreference.bind(0, callback))
    });
}
...
function doInit(choice, callback) {
    ...
    fs.writeFile('awf.conf', content, function() {
...
function warnExists(fpath, callback) {
    fs.exists(fpath, function(err, exists) {
        if (err || exists) {
            return onWarn(fpath + ' already exists or inaccessible', callback);
        }
        callback();
    });
}
