...
function init(callback) {
    ...
    fs.exists(path.join(targetDir, targetName + '.js'), function(err, exists) {
        ...
        fs.exists(path.join(targetDir, targetName + '.css'), function(err, exists) {
            if (err || exists) {
                return warn(targetName + '.css already exists or inaccessible');
            }
...
function choosePreference(callback) {
    prompt.get([{
        ...
        if (err) return onErr(err);
        fs.exists(choice.build_dir, function(err, exists) {
            if (err || exists) {
                console.log(choice.build_dir + ' exists or inaccessible');
                prompt.get([{
                    name: 'go',
                    description: 'continue anyway',
                    default: 'no',
