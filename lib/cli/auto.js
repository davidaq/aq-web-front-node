...
pipeline.runFor(['json'], function(item, callback) {
    //compile(item, new Compiler(require('../compile-func/json')), callback);
...
pipeline.runFor('@link', function(item, callback) {
    dir.all('awfco').then(function(list) {
        ...
        require('../link')(files, cfg.linkTarget, callback)
    }).catch(function(err) {
...
module.exports = function(callback) {
    ...
    dir.onchange('conf', function(fnode) {
        if (fnode.path == 'awf.conf') {
            fs.readFile(fnode.path, 'utf-8', function(err, content) {
                if (!err && content) {
