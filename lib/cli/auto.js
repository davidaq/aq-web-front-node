...
pipeline.runFor(['js','jsx.html'], function(item, callback) {
    if (item == '_build/app.js')
        return callback();
    console.log('Compile', item);
    var vname = item.replace(/\.jsx\.html$/i, '.js');
    var target = path.join('_build', fsext.changeExt(vname, 'jsc'));
...
module.exports = function(callback) {
    ...
    dir = require('../dir');
    dir.onchange(['jsc','info','js','jsx.html'], function(fnode) {
