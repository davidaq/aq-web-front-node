            ...
            var server = tcp.createServer({
                ...
                client.pipe(concat(function(data) {
                    if (data.length == 3 && data[0] == 0) {
                        ...
                        var compiler = fork.openCompiler(job.compilerPath);

                        if (job.inOpt) {
                            compiler.send(job, client);
                        } else {
                            client.end();
                        }
                    }
 ...
 */
function compile(compilerPath, inOpt, success, reject) {
    start(function(port) {
        var client = tcp.connect({port:port,allowHalfOpen:true}, function() {
            ...
            if (inOpt) {
                client.pipe(concat(function(result) {
                    result = JSON.parse(result);
                    var cb = result.success ? success : reject;
                    cb.apply(null, result.args);
                }));
            }
        });
    });
