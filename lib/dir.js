"use strict";

var path = require('path');
var EventEmitter = require('events').EventEmitter;
var chokidar = require('chokidar');

var fnodes = {};
var ev = new EventEmitter();
var ready = false;
var watcher;

(function watch() {
    watcher = chokidar.watch('.', {
        ignored: isIgnored,
        persistent: true,
        depth: 9,
        ignorePermissionErrors: true,
        atomic: true
    }).on('add', function(fpath, stat) {
        var fname = path.basename(fpath);
        var item = {};
        item.path = fpath;
        item.name = fname;
        if (stat) {
            item.time = Math.max(stat.ctime.getTime(), stat.mtime.getTime());
        } else {
            item.time = new Date().getTime();
        }
        item.ext = {};

        var portion = fname.split('.');
        for (var i = 1; i < portion.length; i++) {
            item.ext[portion[i]] = 1;
        }
        fnodes[fpath] = item;
        signalChange(item);
    }).on('change', function(fpath) {
        var fnode = fnodes[fpath];
        if (fnode) {
            fnode.time = new Date().getTime();
            signalChange(fnode);
        }
    }).on('unlink', function(fpath) {
        var fnode = fnodes[fpath];
        if (fnode) {
            delete fnodes[fpath];
            ev.emit('unlinked', fnode);
        }
    }).on('ready', function() {
        ready = true;
        ev.emit('ready');
    });
})();

function signalChange(fnode) {
    if (fnode.changeDelay) {
        clearTimeout(fnode.changeDelay);
    }
    fnode.changeDelay = setTimeout(function() {
        fnode.changeDelay = false;
        ev.emit('changed', fnode);
    }, 10);
}

function all(ext) {
    function result() {
        var ret = []
        for (var fpath in fnodes) {
            var fnode = fnodes[fpath];
            if (matchExt(fnode, ext)) {
                ret.push(fnode);
            }
        }
        return ret;
    }
    return new Promise(function(resolve, reject) {
        if (ready) {
            resolve(result());
        } else {
            ev.on('ready', function() {
                resolve(result());
            });
        }
    });
}

function onchange(ext, func) {
    onevent(ext, func, 'changed');
}

function onunlink(ext, func) {
    onevent(ext, func, 'unlinked');
}

function onevent(ext, func, evname) {
    if (ext instanceof Array) {
        for(var i = 0; i < ext.length; i++) {
            onevent(ext[i], func, 'changed');
        }
    } else {
        if (typeof ext == 'function') {
            func = ext;
            ext = null;
        }
        ev.on(evname, function(fnode) {
            if (matchExt(fnode, ext)) {
                func(fnode);
            }
        });
    }
}

function matchExt(fnode, ext) {
    if (ext) {
        if (ext instanceof Array) {
            for (var i = 0; i < ext.length; i++) {
                if (fnode.ext[ext[i]]) {
                    return true;
                }
            }
        } else if(fnode.ext[ext]) {
            return true;
        }
    } else {
        return true;
    }
    return false;
}

function fetch(fpath) {
    return fnodes[fpath] || null;
}

function close() {
    watcher.close();
}

var ignores = {
    'node_modules':1,
    'log':1
};
function isIgnored(fpath) {
    if (fpath == '.')
        return false;
    var fname = path.basename(fpath);
    if (fname[0] == '.' || fname in ignores) {
        return true;
    }
    return false;
}

module.exports = {
    all:all,
    onchange:onchange,
    onunlink:onunlink,
    fetch:fetch,
    close:close
};

