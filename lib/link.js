...
Link.prototype.fetchLibs = function() {
    ...
    useLibList.map(function(k) {
        ...
        if (lib && lib.inline) {
            ...
            downloadLib(url, self.conf.build_dir, function(content) {
                if (!content) {
...
Link.prototype.combine = function() {
    ...
    for (var k in this.useLib) {
        if (builtIn[k]) {
            ...
            var lib = this.libs[k];
            if (!lib.inline) {
                this.jsOut.writeNoMap('$$$AWF$$$.lib[' + JSON.stringify(k) + ']=' + JSON.stringify({
                    url: this.conf.debug ? lib.debug || lib.url : lib.url,
                    exports: lib.exports
                }) + ';');
                this.jsOut.writeNoMap('$$$AWF$$$.use[' + JSON.stringify(k) + ']=1;')
            }
            this.jsOut.writeNoMap('$$$AWF$$$.define(' + JSON.stringify(k) + ', function(module, exports, require) {');
            if (lib.inline) {
            ...
            if (lib.global && lib.exports.match(/^[a-z_$][a-z0-9_$]*$/i)) {
                this.jsOut.writeNoMap('var ' + lib.exports + '=$$$AWF$$$.require(' + JSON.stringify(k) + ');');
            }
        }
    }    
    var self = this;
