...
function link(flist, target, callback) {
    ...
    function combine(sorted) {
        ...
        function combineMapAdd(sources, map) {
            var baseIndex = sourceIndex;
            for (var i = 0; i < sources.length; i++) {
                var source = sources[i];
                combineMap.sources[sourceIndex] = source.path;
                combineMap.sourcesContent[sourceIndex] = source.content;
                sourceIndex++;
            }
        ...
        function nextMod() {
            ...
            if (!mod) {
                ...
                fsext.unpack(mod.compileFile, {
                    ...
                    if (content.map) {
                        combineMapAdd(content.sources || [{
                            path:mod.relativePath, content:content.sourceContent
                        }], content.map);
                    } else {
