...
Link.prototype.sort = function() {
    ...
    function checkDepMod(mod, dep, depName) {
        var depMod = mods[dep];
        if(depMod) {
            if (depMod.loaded) {
                mod.wait--;
                delete mod.dep[depName || dep]
            } else {
    ...
    _.sortBy(_.values(mods), 'importPath').map(function check(mod) {
        ...
        _(mod.dep).each(function(__, dep) {
            if (dep[0] == '/') {
                checkDepMod(mod, dep);
            } else if (libs[dep] || builtIn[dep]) {
                mod.wait--;
                delete mod.dep[dep];
                useLib[dep] = 1;
            } else {
                checkDepMod(mod, '/' + dep, dep);
            }
        });
...
Link.prototype.combine = function() {
    ...
    for (var k in this.libs) {
        if (!builtIn[k] && this.useLib[k]) {
            ...
            if (lib.inline) {
                ...
                this.jsOut.writeNoMap('if(exports.$$$AWF$$$Extended)return;exports.$$$AWF$$$Extended=true');
            }
            if (libExtend[k]) {
    ...
    function nextMod() {
        ...
        if (!mod) {
            var waitAll = _.after(4, function() {
