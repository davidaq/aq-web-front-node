            ...
            React.createElement = function() {
                ...
                if (options && typeof options == 'object') {
                    ...
                    if (options.className 
                        ...
                        && typeof options.className.join == 'function') {
    ...
    $$$AWF$$$.XMLHttpRequest = (function() {
        ...
        function parseUrl(url) {
            ...
            if (!parsedUrl.protocol) {
                var origin = document.location.href.match(/https?:\/\/([^\/]*)/i)[0];
                if (url.substr(0, 1) == '/') {
                    parsedUrl.href = origin + url;
                } else {
                    var dir = document.location.href.substr(origin.length).replace(/\/[^\/]*(\?.*)?(#.*)?$/, '/');
                    parsedUrl.href = origin + dir + url;
                }
            }
            return normalizeParse(parsedUrl);
        }
        function normalizeParse(parse) {
            return {
                hash: parse.hash ? (parse.hash.substr(0, 1) == '#' ? parse.hash : '#' + parse.hash) : '',
                host: parse.host,
                hostname: parse.hostname,
                href: parse.href,
                origin: parse.origin || parse.protocol + '//' + parse.host,
                pathname: parse.pathname.substr(0, 1) == '/' ? parse.pathname : '/' + parse.pathname,
                port: parse.port,
                protocol: parse.protocol,
                search: parse.search ? (parse.search.substr(0, 1) == '?' ? parse.search : '?' + parse.search) : ''
            };
        }
        return Request;
    })();
