...
function BEGIN() {
    ...
    if (loc.hash && loc.hash.match(/^\#?DEBUG$/))
        window.DEBUG = true;
    if (!document.head) {
    ...
    function ready() {
        if (typeof React == 'object' && typeof React.createElement != 'undefined') {
...
$$$AWF$$$.common._instanceof = function(left, right) {
	if ($$$AWF$$$.legacyBrowser) {
		while (left) {
			if (left == right || left instanceof right) {
				return true;
			}
...
if (typeof document != 'undefined' && typeof window != 'undefined') {
    ...
    if ($$$AWF$$$.legacyBrowser) {
        var extern = $$$AWF$$$.extern;
        extern.push({url:'https://cdnjs.cloudflare.com/ajax/libs/es5-shim/4.4.1/es5-shim.min.js'});
        extern.push({url:'https://cdnjs.cloudflare.com/ajax/libs/es5-shim/4.4.1/es5-sham.min.js'});
        extern.push({url:'https://cdnjs.cloudflare.com/ajax/libs/json3/3.3.2/json3.min.js'});
    }
    ...
    (function(readyFunc) {
        ...
        if (document.addEventListener) {
            ...
            if (document.documentElement.doScroll && !window.frameElement) doScrollCheck();
        }
    ...
    $$$AWF$$$.XMLHttpRequest = (function() {
        ...
        Request.invoke = function(method, url, send, callback, resultType) {
            ...
            if (typeof send == 'object') {
                if (method == 'GET') {
                    ...
                    send = urlencode(parseQuery(parsedUrl.search, send));
                    if (send) {
                        url += '?' + send;
                    }
                    send = '';
        ...
        var jsonpCounter = 0;
        Request.jsonp = function(url, send, callback) {
            if (typeof send == 'function') {
                callback = send;
                send = {};
            }
            var parsed = parseUrl(url);
            send = parseQuery(parsed.search, send);
            var cbname = '$$$AWF$$$jsonP' + new Date().getTime() + '_' + (++jsonpCounter);
            for (var k in send) {
                if (send[k] == '?') {
                    send[k] = cbname;
                }
            ...
            if (send)
                send = '?' + send;
            url = parsed.origin + parsed.pathname + send;
            window[cbname] = callback;
            $$$AWF$$$.loadScript(url, function(arg, loader) {
        ...
        function parseQuery(search, obj) {
            var parsed = {};
