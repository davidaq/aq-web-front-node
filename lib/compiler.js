        ...
        fs.readFile(fpath, 'utf-8', function(err, content) {
            if (err) {
                ...
                function success(output, map, dependency) {
                    ...
                    result.map = [];
                    var lastLine;
                    result.map.push(lastLine = [[0,0,0,0]]);
                    _(map).each(function(line) {
                        _(line).each(function(item) {
                            if (item.length > 4)
                                item.splice(4);
                        });
                        result.map.push(lastLine = line);
                    });
                    result.map.push(lastLine);
                    result.dep = dependency || [];
                    result.sourceContent += '\n// ' + result.relativePath + '\n';
                    fsext.pack(target, result).then(accept);
                }
                function error(msg, line, column) {
                    if (msg instanceof Array) { 
                        var output = 'module.exports = null;\n';
                        var map = [[[0, 0, 0, 0]]];
