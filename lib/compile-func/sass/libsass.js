...
function Worker(script) {
    ...
    var child = this.child = cp.fork(script);
    process.on('beforeExit', function() {
        child.kill();
    });
}

Worker.prototype.addEventListener = function(evtName, handler) {
    if (evtName == 'message') {
        this.child.on(evtName, function(data) {
            handler({data:data});
        });
    }
};
...
Sass.setWorkerUrl(require.resolve('./libsass.worker'));
return Sass;
}));
